...
pub(super) fn minify_html_content(mut rendered: String) -> Result<String> {
    let new_len =
        minify_html_onepass::in_place(unsafe { rendered.as_bytes_mut() }, minify_html_cfg())
            .map_err(|e| {
                miette!(
                    "{} at position {}: {:?}",
...
fn minify_js_asset(src_path: &Path) -> Result<String> {
    let source = std::fs::read_to_string(src_path)
        .into_diagnostic()
        .wrap_err("Failed to read JS asset")?;
...
fn minify_css_asset(src_path: &Path) -> Result<String> {
    let content = std::fs::read_to_string(src_path)
        .into_diagnostic()
        .wrap_err("Failed to read CSS asset")?;
    ...
    let minified = stylesheet
        ...
        .into_diagnostic()
        .wrap_err("Failed to serialize minified CSS")?;
...
fn copy_binary_asset(src_path: &Path, dest_path: &Path) -> Result<()> {
    let content = std::fs::read(src_path)
        .into_diagnostic()
        .wrap_err("Failed to read asset file")?;
